...
internal class RoleManagerService : IRoleManagerService
{
    ...
    public RoleManagerService(AppRoleManager roleManger, IMapper mapper,
        ...
        ILogger<RoleManagerService> logger, ApplicationDbContext db, EndpointDataSource endpointDataSource)
    {
    ...
    {
        var result = await _roleManger.Roles.Where(c => !c.Name.Equals("admin"))
    ...
    public async Task<bool> ChangeRolePermissionsAsync(EditRolePermissionsDto model)
    {
        ...
        if (result.Succeeded)
        {
            ...
            foreach (var user in users) await _userManager.UpdateSecurityStampAsync(user);
